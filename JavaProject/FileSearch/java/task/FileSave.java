...
public class FileSave implements ScanCallback
{
    ...
    public void callback(File dir)
    {
        ...
        获取本地目录下一级子文件和文件夹
        集合框架中使用自定义类型,判断是否某个对象在集合内存在:比对两个集合中的元素
        list,set
         */
        ...
        for(FileMeta meta: metas)
        {
            ...
            {
                //meta的删除 1.删除meta信息本身  2.如果meta是目录,还要将meta所有的子文件,子文件夹都删除
                delete(meta);
            }
        }
    ...
    private List<FileMeta> query(File dir)
    {
        ...
        {
            e.printStackTrace();
            throw new RuntimeException("查询文件信息出错,需要检查sql查询语句",e);
        }
        finally
        {
            //释放资源
    ...
    private void save(FileMeta meta)
    {
        ...
        try
        {
            //1.获取数据库连接
            connection = DBUtil.getConnection();
            //添加is_directory, 占位符, set值3, 4, 5, 6 + 1,添加set第3个参数is_direcotry
        ...
        {
            e.printStackTrace();
            throw new RuntimeException("文件保存失败,请检查 sql insert 语句",e);
        }
        finally
        {
            //4.释放资源
    ...
    public void delete(FileMeta meta)
    {
        ...
        try
        {
            ...
            {
                sql += "or path=?" + "or path like ?"; //匹配数据库文件夹的子文件夹和类似路径的子孙后辈文件夹
            }
        ...
        {
            e.printStackTrace();
            throw new RuntimeException("删除文件信息出错,请检查delete语句");
        }
        finally
        {
